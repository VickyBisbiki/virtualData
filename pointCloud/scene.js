var dataPoints = (function(){
  ...
  var instansiatePoints = {};
  var animation;

  instansiatePoints.init  = function (scene) {
    // Animation extends THREE.Mesh
    animation = new Animation();
    animation.name = "objAnimation";
    animation.animate(8.0, {ease: Power0.easeIn, repeat:0, repeatDelay:1.0, yoyo: true});
    scene.add(animation);
};
...
function Animation() {
  ...
  var prefabGeometry = new THREE.TetrahedronGeometry(0.004);
  ...
  for (i = 0; i < prefabCount; i++) {
    startPosition.x = THREE.Math.randFloatSpread(range*instansiatePoints.mulRangeStart) + instansiatePoints.xPosStart;
    startPosition.y = THREE.Math.randFloatSpread(range*instansiatePoints.mulRangeStart) + instansiatePoints.yPosStart;
    startPosition.z = THREE.Math.randFloatSpread(range*instansiatePoints.mulRangeStart) + instansiatePoints.zPosStart;

    endPosition.x = THREE.Math.randFloatSpread(range*instansiatePoints.mulRangeEnd) + instansiatePoints.xPosEnd;
    endPosition.y = THREE.Math.randFloatSpread(range*instansiatePoints.mulRangeEnd) + instansiatePoints.yPosEnd;
    endPosition.z = THREE.Math.randFloatSpread(range*instansiatePoints.mulRangeEnd) + instansiatePoints.zPosEnd;
