var dataPoints02 = (function(){

  var instansiatePoints02 = {};
  var animation02;

  instansiatePoints02.init  = function (scene) {
    // Animation extends THREE.Mesh
    animation02 = new Animation();
    animation02.name = "objAnimation02";
    animation02.animate(8.0, {ease: Power0.easeIn, repeat:0, repeatDelay:0.1, yoyo: true});
    scene.add(animation02);
};
...
function Animation() {
  ...
  for (i = 0; i < prefabCount; i++) {
    startPosition.x = THREE.Math.randFloatSpread(range*instansiatePoints02.mulRangeStart02) + instansiatePoints02.xPosStart;
    startPosition.y = THREE.Math.randFloatSpread(range*instansiatePoints02.mulRangeStart02) + instansiatePoints02.yPosStart;
    startPosition.z = THREE.Math.randFloatSpread(range*instansiatePoints02.mulRangeStart02) + instansiatePoints02.zPosStart;

    endPosition.x = THREE.Math.randFloatSpread(range*instansiatePoints02.mulRangeEnd02) + instansiatePoints02.xPosEnd;
    endPosition.y = THREE.Math.randFloatSpread(range*instansiatePoints02.mulRangeEnd02) + instansiatePoints02.yPosEnd;
    endPosition.z = THREE.Math.randFloatSpread(range*instansiatePoints02.mulRangeEnd02) + instansiatePoints02.zPosEnd;
  ...
  geometry.createAttribute('aAxisAngle', 4, function(data, i, total) {
    // get a random axis
    axis.x = THREE.Math.randFloatSpread(0.2);
    axis.y = THREE.Math.randFloatSpread(0.2);
    axis.z = THREE.Math.randFloatSpread(0.2);
    // axis has to be normalized, or else things get weird
    axis.normalize();
    // the total angle of rotation around the axis
    angle = Math.PI * THREE.Math.randFloat(0.4, 0.8);
  ...
  var material = new THREE.BAS.StandardAnimationMaterial({
    ...
    vertexPosition: [
      ...
      'float scl = easeQuadOut(tProgress, 0.8, 0.8, 0.7);',
